...
- SSRF: hostname validation, metadata/localhost/link-local screening in scope validation (§83).
...
- Secret-shaped banner values redacted before storage (§127).
- Request body caps (10 MiB), event payload caps (256 KiB stream / 8 MiB API), oversized records dropped and logged (§104).
- Rate limits on auth, ingest, expensive endpoints; never on read-only table navigation.
- Audit log (§85): logins, token revocations, scans (incl. elevated with reason), scope changes, agent enrollment/tasks, finding status changes, report generation — who, what, when, from where, result.

## Deployment hardening (§155)
